...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `src/web_service.py` um Request-Korrelation für `/analyze` erweitert (erste **gültige** ID aus `X-Request-Id`/`X-Correlation-Id` wird in Response-Header + JSON-Feld `request_id` gespiegelt; leere/whitespace-only IDs, IDs mit eingebettetem Whitespace, IDs mit Steuerzeichen oder IDs mit mehr als 128 Zeichen werden verworfen) für reproduzierbare Remote-Diagnosen.
  ...
  - `tests/test_web_e2e.py` um API-E2E-Guards erweitert: ist `X-Request-Id` leer/whitespace, enthält eingebetteten Whitespace, enthält Steuerzeichen (z. B. Tab) **oder ist länger als 128 Zeichen**, fällt der Service deterministisch auf `X-Correlation-Id` zurück und spiegelt diese ID in Header+JSON.
  ...
  - `scripts/run_remote_api_smoketest.sh` unterstützt jetzt `SMOKE_REQUEST_ID_HEADER=request|correlation|x-request-id|x-correlation-id|x_request_id|x_correlation_id` (default `request`) und erlaubt damit reproduzierbare Remote-Fallback-Checks über `X-Correlation-Id`; Header-/Echo-Flags werden vor Validierung zusätzlich getrimmt, Header-Namen werden als Alias normalisiert und ungültige Modi bleiben fail-fast mit `exit 2`. Zusätzlich weist der Runner `SMOKE_REQUEST_ID_HEADER` jetzt auch für whitespace-only Werte, eingebettete Whitespaces und Steuerzeichen deterministisch mit `exit 2` zurück.
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt in Worker-1-10m Iteration 32 mit getrimmten Env-Inputs, Header-Aliasen (`SMOKE_REQUEST_ID_HEADER="X_Request_Id"` bzw. `"X_Correlation_Id"`) und den neuen Header-Mode-Guards durchgeführt; Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772112911.json` + `artifacts/worker-1-10m/iteration-32/bl18.1-remote-stability-local-worker-1-10m-1772112911.ndjson` (`pass=5`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772112911.log`.
  ...
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`96 passed`, Exit `0`) direkt vor dem dedizierten Worker-1-10m-Langlauf (Smoke + 5x Stabilität).
