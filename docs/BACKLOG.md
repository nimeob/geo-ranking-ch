...
- ⏸️ blockiert (2026-03-03): [#865](https://github.com/nimeob/geo-ranking-ch/issues/865) — DEV-VERIFY-0: Dev End-to-End Verification (EPIC #800 DoD) — OIDC-Auth funktioniert (`/healthz` und synchrones `/analyze` grün), aber Async-Path bleibt blockiert: OIDC-Job bleibt >120s `queued`, `/analyze/history` bleibt leer (`total=0`). Follow-up erstellt: [#924](https://github.com/nimeob/geo-ranking-ch/issues/924). Evidence: `reports/evidence/issue-865-blocker-async-oidc-20260303T131549Z.json`.
