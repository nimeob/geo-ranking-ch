...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `tests/test_remote_smoke_script.py` ergänzt (lokale E2E-Validierung des Smoke-Skripts inkl. Auth-Pfad/Fehlpfad + Request-ID-Echo-Nachweis) und um Happy-Paths für `DEV_BASE_URL=.../health`, verkettete Suffixe (`.../health/analyze`), case-insensitive Suffixe (`.../HeAlTh/AnAlYzE`), getrimmte Whitespace-Inputs (`"  http://.../health  "`) sowie grossgeschriebenes HTTP-Schema (`HTTP://...`) erweitert (URL-Normalisierung + Schema-Handling auf `/analyze` reproduzierbar abgesichert).
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-C-Langlauf-Recheck mit gemischtem Input-Pfad `DEV_BASE_URL="HTTP://127.0.0.1:18086/HeAlTh"` (Schema großgeschrieben + case-insensitive Suffix) und Evidenz in `artifacts/bl18.1-smoke-local-worker-c-1772092067.json` + `artifacts/bl18.1-remote-stability-local-worker-c-1772092067.ndjson` (`pass=3`, `fail=0`). Frühere bestätigte Läufe bleiben nachvollziehbar dokumentiert (`worker-a`, `worker-b`, `worker-c`, `worker-a-isolated`, `worker-b2`, `worker-b-langlauf`, `worker-a-whitespace`, `worker-b-suffix-case`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`28 passed`, Exit `0`) direkt vor dem dedizierten Worker-C-Langlauf-Recheck (Smoke + 3x Stabilität, `HTTP://.../HeAlTh`-Input).
