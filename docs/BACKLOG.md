...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-1-Langlauf (Iteration 6) mit getrimmtem Correlation-Header-Mode (`SMOKE_REQUEST_ID_HEADER="  Correlation  "`), getrimmtem Echo-Flag (`SMOKE_ENFORCE_REQUEST_ID_ECHO=" 1 "`), getrimmten Timeout-Inputs (`SMOKE_TIMEOUT_SECONDS=" 2.5 "`, `CURL_MAX_TIME=" 15 "`), getrimmten Retry-Flags (`CURL_RETRY_COUNT=" 1 "`, `CURL_RETRY_DELAY=" 1 "`) und verketteter Base-URL (`DEV_BASE_URL="  HTTP://127.0.0.1:59099/health/analyze/health/analyze///  "`), Evidenz in `artifacts/bl18.1-smoke-local-worker-1-langlauf-1772098485.json` + `artifacts/bl18.1-remote-stability-local-worker-1-langlauf-1772098485.ndjson` (`pass=5`, `fail=0`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`59 passed`, Exit `0`) direkt vor dem dedizierten Worker-1-Langlauf (Smoke + 5x Stabilität, Trim-Recheck für Timeout-/Retry-Flags + `SMOKE_MODE`).
