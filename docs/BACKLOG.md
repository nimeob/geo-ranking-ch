...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt in Worker-1-10m Iteration 29 mit getrimmten Env-Inputs (`DEV_BASE_URL`, `DEV_API_AUTH_TOKEN`, `SMOKE_MODE`, `SMOKE_QUERY`, Header-/Echo-Flags) durchgeführt; Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772111118.json` + `artifacts/worker-1-10m/iteration-29/bl18.1-remote-stability-local-worker-1-10m-1772111118.ndjson` (`pass=5`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772111118.log`.
