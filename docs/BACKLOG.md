...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `tests/test_remote_smoke_script.py` ergänzt (lokale E2E-Validierung des Smoke-Skripts inkl. Auth-Pfad/Fehlpfad + Request-ID-Echo-Nachweis) und um Happy-Paths für `DEV_BASE_URL=.../health`, verkettete Suffixe (`.../health/analyze`), case-insensitive Suffixe (`.../HeAlTh/AnAlYzE`), getrimmte Whitespace-Inputs (`"  http://.../health  "`), die kombinierte Normalisierung (`"  HTTP://.../HeAlTh/AnAlYzE/  "`), redundante trailing-Slash-Ketten (`.../health//analyze//`) sowie grossgeschriebenes HTTP-Schema (`HTTP://...`) erweitert (URL-Normalisierung + Schema-Handling auf `/analyze` reproduzierbar abgesichert).
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-B-Langlauf mit getrimmter Request-ID (`SMOKE_REQUEST_ID="  bl18-worker-b-langlauf-1772093324  "`) und Evidenz in `artifacts/bl18.1-smoke-local-worker-b-langlauf-1772093324.json` + `artifacts/bl18.1-remote-stability-local-worker-b-langlauf-1772093324.ndjson` (`pass=3`, `fail=0`, Stability-Run-IDs mit PID-Suffix).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`36 passed`, Exit `0`) direkt vor dem dedizierten Worker-B-Langlauf (Smoke + 3x Stabilität, Report-Guard + Request-ID-Entkopplung).
