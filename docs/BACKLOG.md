...
- **Umgesetzt (Iteration 2026-02-26, historisch):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Korrelation bestätigt; zuletzt in Worker-1-10m Iteration 45 mit getrimmten Env-Inputs im Short-Alias-Mode (`SMOKE_REQUEST_ID_HEADER="request-id"` im Smoke, `"correlation_id"` in Stabilität), getrimmtem `SMOKE_MODE="RiSk"` und booleschen Flag-Aliasen (`SMOKE_ENFORCE_REQUEST_ID_ECHO="TrUe"` im Smoke, `SMOKE_ENFORCE_REQUEST_ID_ECHO="FaLsE"` + `STABILITY_STOP_ON_FIRST_FAIL="fAlSe"` in Stabilität) durchgeführt. Der Smoke lief diesmal bewusst ohne gesetzte `SMOKE_REQUEST_ID`, sodass die neue auto-generierte Default-ID (`bl18-<epoch>-<uuid-suffix>`) im Real-Run verifiziert wurde; Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772120889.json` + `artifacts/worker-1-10m/iteration-45/bl18.1-remote-stability-local-worker-1-10m-1772120889.ndjson` (`pass=3`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772120889.log`.
