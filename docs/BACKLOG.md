...
- **Umgesetzt (Iteration 2026-02-26, historisch):**
  ...
  - `src/web_service.py` normalisiert die Routenauflösung jetzt robust über den URL-Pfad (`urlsplit(...).path`), ignoriert Query-/Fragment-Anteile für die Endpunktwahl und toleriert optionale trailing Slashes (`/health/?...`, `/version/?...`, `/analyze/?...`); `tests/test_web_e2e.py` deckt die neuen Pfadfälle reproduzierbar ab.
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Korrelation bestätigt; zuletzt in Worker-1-10m Iteration 47 mit getrimmten Env-Inputs im Short-Hyphen-Correlation-Mode (`SMOKE_REQUEST_ID_HEADER="correlation-id"` im Smoke) und Underscore-`X`-Request-Mode in der Stabilität (`SMOKE_REQUEST_ID_HEADER="x_request_id"`), inklusive boolescher Flag-Aliasse (`SMOKE_ENFORCE_REQUEST_ID_ECHO="on"` im Smoke, `SMOKE_ENFORCE_REQUEST_ID_ECHO="No"` + `STABILITY_STOP_ON_FIRST_FAIL="off"` in Stabilität). Zusätzlich wurde API-seitig die Pfad-Normalisierung live verifiziert (`/health/?probe=1`, `/analyze/?trace=path-normalization` → jeweils `200` + konsistentes Request-ID-Echo in Header+JSON). Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772121986.json`, `artifacts/worker-1-10m/iteration-47/bl18.1-remote-stability-local-worker-1-10m-1772121986.ndjson`, `artifacts/bl18.1-path-normalization-worker-1-10m-1772121986.json` (`pass=3`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772121986.log`.
  ...
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`122 passed`, Exit `0`) direkt vor dem dedizierten Worker-1-10m-Langlauf (Iteration 47: Smoke + 3x Stabilität + API-Pfad-Realcheck).
