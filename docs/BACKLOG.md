...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-A-Langlauf mit getrimmter Request-ID (`SMOKE_REQUEST_ID="  bl18-worker-a-1772093014  "`) und Evidenz in `artifacts/bl18.1-smoke-local-worker-a-1772093014.json` + `artifacts/bl18.1-remote-stability-local-worker-a-1772093014.ndjson` (`pass=3`, `fail=0`). Frühere bestätigte Läufe bleiben nachvollziehbar dokumentiert (`worker-a`, `worker-b`, `worker-c`, `worker-a-isolated`, `worker-b2`, `worker-b2-recheck`, `worker-b-langlauf`, `worker-a-whitespace`, `worker-b-suffix-case`, `worker-c-trailing-slash`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`35 passed`, Exit `0`) direkt vor dem dedizierten Worker-A-Langlauf (Smoke + 3x Stabilität, Request-ID-Trim-Echo).
