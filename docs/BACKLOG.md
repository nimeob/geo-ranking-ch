...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-A-Langlauf (Iteration 9) mit Tab-getrimmtem Correlation-Header-Mode (`SMOKE_REQUEST_ID_HEADER=$' \tCorrelation\t '`), getrimmtem Echo-Flag (`SMOKE_ENFORCE_REQUEST_ID_ECHO=$' \t1\t '`), case-insensitive normalisiertem `SMOKE_MODE=$' \tExTenDeD\t '`, getrimmten Timeout-/Retry-Flags (`SMOKE_TIMEOUT_SECONDS=$'\t2.5\t'`, `CURL_MAX_TIME=" 15 "`, `CURL_RETRY_COUNT=$'\t1\t'`, `CURL_RETRY_DELAY=$'\t1\t'`) und kombinierter Suffix-Kette (`DEV_BASE_URL=$' \tHTTP://127.0.0.1:43577/AnAlYzE//health/analyze/health/analyze///\t '`), Evidenz in `artifacts/bl18.1-smoke-local-worker-a-1772099418.json` + `artifacts/bl18.1-remote-stability-local-worker-a-1772099418.ndjson` (`pass=5`, `fail=0`).
