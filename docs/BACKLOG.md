...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt in Worker-A Iteration 28 mit Timeout-Konsistenz-Guard (`CURL_MAX_TIME >= SMOKE_TIMEOUT_SECONDS`) und getrimmten Env-Inputs (`DEV_BASE_URL`, `DEV_API_AUTH_TOKEN`, `SMOKE_MODE`, `SMOKE_QUERY`, Header-/Echo-Flags) durchgeführt; Evidenz in `artifacts/bl18.1-smoke-local-worker-a-1772109929.json` + `artifacts/bl18.1-remote-stability-local-worker-a-1772109929.ndjson` (`pass=3`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-a-server-1772109929.log`.
