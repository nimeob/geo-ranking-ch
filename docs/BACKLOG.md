...
- ✅ Follow-up verifiziert (2026-03-03): [#865](https://github.com/nimeob/geo-ranking-ch/issues/865) — DEV-VERIFY-0: Der OIDC-Async-Blocker aus [#924](https://github.com/nimeob/geo-ranking-ch/issues/924) ist behoben und auf dev verifiziert (`build=37ca5a0`): `POST /analyze` (async) läuft `queued -> running -> completed`, `/analyze/history` enthält das erzeugte `result_id`. Evidence: `reports/evidence/issue-924-oidc-async-verify-20260303T140858Z.json` (historischer Blocker: `reports/evidence/issue-865-blocker-async-oidc-20260303T131549Z.json`).
