...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `scripts/run_remote_api_smoketest.sh` trimmt optionales `DEV_API_AUTH_TOKEN` jetzt vor dem Request und weist whitespace-only Tokenwerte fail-fast mit `exit 2` zurück; `tests/test_remote_smoke_script.py` ergänzt dafür einen Happy-Path mit Tab/Space-umhülltem Token sowie einen Negativtest für whitespace-only Token.
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-A-Langlauf (Iteration 11) im getrimmten `request`-Header-Mode (`SMOKE_REQUEST_ID_HEADER="  request  "`), getrimmtem Echo-Flag (`SMOKE_ENFORCE_REQUEST_ID_ECHO=" 1 "`), case-insensitive normalisiertem `SMOKE_MODE="  rIsK  "`, getrimmtem optionalen Bearer-Token (`DEV_API_AUTH_TOKEN="  bl18-token\t"`), getrimmten Timeout-/Retry-/Stability-Flags sowie kombinierter Suffix-Kette (`DEV_BASE_URL="  HTTP://127.0.0.1:59917/health/analyze//health/analyze///  "`), Evidenz in `artifacts/bl18.1-smoke-local-worker-a-1772100333.json` + `artifacts/bl18.1-remote-stability-local-worker-a-1772100333.ndjson` (`pass=5`, `fail=0`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`64 passed`, Exit `0`) direkt vor dem dedizierten Worker-A-Langlauf (Smoke + 5x Stabilität mit getrimmtem `DEV_API_AUTH_TOKEN`).
