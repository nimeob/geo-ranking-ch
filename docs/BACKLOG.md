...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-B-Langlauf mit case-insensitive Suffix-Pfad `DEV_BASE_URL="http://127.0.0.1:49115/HeAlTh/AnAlYzE"` und Evidenz in `artifacts/bl18.1-smoke-local-worker-b-1772091885.json` + `artifacts/bl18.1-remote-stability-local-worker-b-1772091885.ndjson` (`pass=3`, `fail=0`). Frühere bestätigte Läufe bleiben nachvollziehbar dokumentiert (`worker-a`, `worker-b`, `worker-c`, `worker-a-isolated`, `worker-b2`, `worker-b-langlauf`, `worker-a-whitespace`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`28 passed`, Exit `0`) direkt vor dem dedizierten Worker-B-Langlauf (Smoke + 3x Stabilität, case-insensitive Suffix-Pfad).
