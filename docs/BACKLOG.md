...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `tests/test_remote_smoke_script.py` ergänzt (lokale E2E-Validierung des Smoke-Skripts inkl. Auth-Pfad/Fehlpfad + Request-ID-Echo-Nachweis) und um Happy-Paths für `DEV_BASE_URL=.../health`, verkettete Suffixe (`.../health/analyze`), gemischte Suffix-Reihenfolge (`.../analyze/health//`), wiederholte Suffix-Ketten (`.../health/analyze/health/analyze///`), case-insensitive Suffixe (`.../HeAlTh/AnAlYzE`), getrimmte Whitespace-Inputs (`"  http://.../health  "`), die kombinierte Normalisierung (`"  HTTP://.../HeAlTh/AnAlYzE/  "`), die kombinierte Reverse-Suffix-Kette (`"  HTTP://.../AnAlYzE/health//  "`), redundante trailing-Slash-Ketten (`.../health//analyze//`) sowie grossgeschriebenes HTTP-Schema (`HTTP://...`) erweitert (URL-Normalisierung + Schema-Handling auf `/analyze` reproduzierbar abgesichert).
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-A-Langlauf-Recheck mit wiederholter Suffix-Kette (`DEV_BASE_URL="http://127.0.0.1:59769/health/analyze/health/analyze///"`) und getrimmter Request-ID (`SMOKE_REQUEST_ID="  bl18-worker-a-langlauf-1772094394  "`), Evidenz in `artifacts/bl18.1-smoke-local-worker-a-langlauf-1772094394.json` + `artifacts/bl18.1-remote-stability-local-worker-a-langlauf-1772094394.ndjson` (`pass=3`, `fail=0`, Stability-Run-IDs mit PID-Suffix).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`40 passed`, Exit `0`) direkt vor dem dedizierten Worker-A-Langlauf-Recheck (Smoke + 3x Stabilität).
