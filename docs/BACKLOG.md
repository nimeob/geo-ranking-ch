...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `tests/test_remote_smoke_script.py` ergänzt (lokale E2E-Validierung des Smoke-Skripts inkl. Auth-Pfad/Fehlpfad + Request-ID-Echo-Nachweis) und um Happy-Paths für `DEV_BASE_URL=.../health`, verkettete Suffixe (`.../health/analyze`), case-insensitive Suffixe (`.../HeAlTh/AnAlYzE`), getrimmte Whitespace-Inputs (`"  http://.../health  "`), die kombinierte Normalisierung (`"  HTTP://.../HeAlTh/AnAlYzE/  "`) sowie grossgeschriebenes HTTP-Schema (`HTTP://...`) erweitert (URL-Normalisierung + Schema-Handling auf `/analyze` reproduzierbar abgesichert).
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-B2-Langlauf mit kombiniertem Input `DEV_BASE_URL="  HTTP://127.0.0.1:57385/HeAlTh/AnAlYzE/  "` (Schema großgeschrieben + case-insensitive Suffixe + trailing Slash + Whitespace) und Evidenz in `artifacts/bl18.1-smoke-local-worker-b2-1772092596.json` + `artifacts/bl18.1-remote-stability-local-worker-b2-1772092596.ndjson` (`pass=3`, `fail=0`). Frühere bestätigte Läufe bleiben nachvollziehbar dokumentiert (`worker-a`, `worker-b`, `worker-c`, `worker-a-isolated`, `worker-b2`, `worker-b2-recheck`, `worker-b-langlauf`, `worker-a-whitespace`, `worker-b-suffix-case`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`31 passed`, Exit `0`) direkt vor dem dedizierten Worker-B2-Langlauf (Smoke + 3x Stabilität, kombinierter `HTTP://.../HeAlTh/AnAlYzE/`-Input).
