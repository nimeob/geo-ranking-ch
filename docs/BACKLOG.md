...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `scripts/run_remote_api_smoketest.sh` unterstützt jetzt `SMOKE_REQUEST_ID_HEADER=request|correlation|x-request-id|x-correlation-id` (default `request`) und erlaubt damit reproduzierbare Remote-Fallback-Checks über `X-Correlation-Id`; Header-/Echo-Flags werden vor Validierung zusätzlich getrimmt, Header-Namen werden als Alias normalisiert und ungültige Modi bleiben fail-fast mit `exit 2`.
  - `tests/test_remote_smoke_script.py` ergänzt Happy-Path-Abdeckung für `SMOKE_REQUEST_ID_HEADER=correlation` sowie Alias-Werte (`"  X-Request-Id  "`, `"\tX-Correlation-Id\t"`) und getrimmte Eingaben (`"  Correlation  "`, `SMOKE_ENFORCE_REQUEST_ID_ECHO=" 1 "`) und enthält weiterhin den Negativtest für ungültige Header-Modi.
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt in Worker-1-10m Iteration 30 mit getrimmten Env-Inputs und Header-Aliasen (`SMOKE_REQUEST_ID_HEADER="X-Request-Id"` bzw. `"X-Correlation-Id"`) durchgeführt; Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772111763.json` + `artifacts/worker-1-10m/iteration-30/bl18.1-remote-stability-local-worker-1-10m-1772111763.ndjson` (`pass=5`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772111763.log`.
  ...
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`89 passed`, Exit `0`) direkt vor dem dedizierten Worker-1-10m-Langlauf (Smoke + 5x Stabilität).
