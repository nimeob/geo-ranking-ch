...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `tests/test_remote_smoke_script.py` ergänzt (lokale E2E-Validierung des Smoke-Skripts inkl. Auth-Pfad/Fehlpfad + Request-ID-Echo-Nachweis) und um Happy-Paths für `DEV_BASE_URL=.../health`, verkettete Suffixe (`.../health/analyze`), getrimmte Whitespace-Inputs (`"  http://.../health  "`) sowie grossgeschriebenes HTTP-Schema (`HTTP://...`) erweitert (URL-Normalisierung + Schema-Handling auf `/analyze` reproduzierbar abgesichert).
  ...
  - `scripts/run_remote_api_smoketest.sh` URL-Normalisierung ergänzt (trimmt führende/trailing Whitespaces, normalisiert `/health`/`/analyze`-Suffixe auch verkettet) + robuste http(s)-Schema-Validierung (inkl. grossgeschriebener Schemata wie `HTTP://`) zur Runbook-Reproduzierbarkeit.
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-A-Langlauf mit getrimmter `DEV_BASE_URL` (`"  http://127.0.0.1:58813/health  "`) und Evidenz in `artifacts/bl18.1-smoke-local-worker-a-1772091687.json` + `artifacts/bl18.1-remote-stability-local-worker-a-1772091687.ndjson` (`pass=3`, `fail=0`). Frühere bestätigte Läufe bleiben nachvollziehbar dokumentiert (`worker-a`, `worker-b`, `worker-c`, `worker-a-isolated`, `worker-b2`, `worker-b-langlauf`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erneut erfolgreich (`27 passed`, Exit `0`) direkt vor dem dedizierten Worker-A-Langlauf (Smoke + 3x Stabilität, Whitespace-Trim-Pfad).
