...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `src/web_service.py` um Request-Korrelation für `/analyze` erweitert (erste **gültige** ID aus `X-Request-Id`/`X-Correlation-Id` wird in Response-Header + JSON-Feld `request_id` gespiegelt; leere/whitespace-only IDs, IDs mit eingebettetem Whitespace oder IDs mit Steuerzeichen werden verworfen) für reproduzierbare Remote-Diagnosen.
  ...
  - `tests/test_web_e2e.py` um API-E2E-Guards erweitert: ist `X-Request-Id` leer/whitespace, enthält eingebetteten Whitespace **oder enthält Steuerzeichen** (z. B. Tab), fällt der Service deterministisch auf `X-Correlation-Id` zurück und spiegelt diese ID in Header+JSON.
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt in Worker-1-10m Iteration 28 mit getrimmten Env-Inputs (`DEV_BASE_URL`, `DEV_API_AUTH_TOKEN`, `SMOKE_MODE`, `SMOKE_QUERY`, Header-/Echo-Flags) durchgeführt; Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772110559.json` + `artifacts/worker-1-10m/iteration-28/bl18.1-remote-stability-local-worker-1-10m-1772110559.ndjson` (`pass=5`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772110559.log`.
  - API-Fallback real verifiziert: eingebetteter Whitespace in `X-Request-Id` wird verworfen und `X-Correlation-Id` deterministisch in Header+JSON gespiegelt (`artifacts/bl18.1-request-id-fallback-worker-1-10m-1772110577.json`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`88 passed`, Exit `0`) direkt vor dem dedizierten Worker-1-10m-Langlauf (Smoke + 5x Stabilität).
