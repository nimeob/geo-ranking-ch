...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Korrelation bestätigt; zuletzt in Worker-1-10m Iteration 43 mit getrimmten Env-Inputs im Short-Alias-Mode (`SMOKE_REQUEST_ID_HEADER="request-id"` bzw. Stabilität `"correlation_id"`), getrimmtem `SMOKE_MODE="RiSk"` und booleschen Flag-Aliasen (`SMOKE_ENFORCE_REQUEST_ID_ECHO="TrUe"` im Smoke, `SMOKE_ENFORCE_REQUEST_ID_ECHO="FaLsE"` + `STABILITY_STOP_ON_FIRST_FAIL="fAlSe"` in Stabilität) durchgeführt; Evidenz in `artifacts/bl18.1-smoke-local-worker-1-10m-1772119525.json` + `artifacts/worker-1-10m/iteration-43/bl18.1-remote-stability-local-worker-1-10m-1772119525.ndjson` (`pass=3`, `fail=0`) inkl. Server-Log `artifacts/bl18.1-worker-1-10m-server-1772119525.log`. Zusätzlich ist der Short-Alias-API-Realcheck dokumentiert (`artifacts/bl18.1-request-id-short-alias-worker-1-10m-1772119525.json`): `Request-Id` wird primär gespiegelt, bei ungültigem primären Request-Alias fällt der Service deterministisch auf `Correlation-Id` zurück.
  ...
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`115 passed`, Exit `0`) direkt vor dem dedizierten Worker-1-10m-Langlauf (Smoke + 3x Stabilität).
