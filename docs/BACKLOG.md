...
- **Umgesetzt (Iteration 2026-02-26, laufend):**
  ...
  - `scripts/run_remote_api_smoketest.sh` trimmt jetzt zusätzlich `SMOKE_MODE`, `SMOKE_TIMEOUT_SECONDS`, `CURL_MAX_TIME`, `CURL_RETRY_COUNT` und `CURL_RETRY_DELAY` vor der Validierung; `tests/test_remote_smoke_script.py` deckt dafür reproduzierbare Happy-Paths mit getrimmtem `SMOKE_MODE="  basic  "`, getrimmten Timeout-Inputs (`SMOKE_TIMEOUT_SECONDS="\t2.5\t"`, `CURL_MAX_TIME=" 15 "`) und getrimmten Retry-Flags (`"\t1\t"`) ab.
  - Real-Run-Nachweis aktualisiert (lokal, 2026-02-26): `run_remote_api_smoketest.sh` Exit `0` + `run_remote_api_stability_check.sh` Exit `0` mit Request-ID-Echo in Header+JSON bestätigt; zuletzt im Worker-C-Langlauf (Iteration 7) mit getrimmtem Correlation-Header-Mode (`SMOKE_REQUEST_ID_HEADER="  Correlation  "`), getrimmtem Echo-Flag (`SMOKE_ENFORCE_REQUEST_ID_ECHO=" 1 "`), getrimmten Timeout-/Retry-Flags (`SMOKE_TIMEOUT_SECONDS=" 2.5 "`, `CURL_MAX_TIME=" 15 "`, `CURL_RETRY_COUNT=" 1 "`, `CURL_RETRY_DELAY=" 1 "`) und verketteter Base-URL (`DEV_BASE_URL="  HTTP://127.0.0.1:39761/analyze/health/analyze///  "`), Evidenz in `artifacts/bl18.1-smoke-local-worker-c-langlauf-1772098788.json` + `artifacts/bl18.1-remote-stability-local-worker-c-langlauf-1772098788.ndjson` (`pass=5`, `fail=0`).
  - Reproduzierbarkeits-Check erneuert: `./scripts/run_webservice_e2e.sh` erfolgreich (`60 passed`, Exit `0`) direkt vor dem dedizierten Worker-C-Langlauf (Smoke + 5x Stabilität, Trim-Recheck für Timeout-/Retry-Flags) sowie zusätzlichem API-Guard-Livecheck (`X-Request-Id` mit Steuerzeichen → deterministischer Fallback auf `X-Correlation-Id`, Evidenz: `artifacts/bl18.1-request-id-control-fallback-worker-c-1772098788.json`).
