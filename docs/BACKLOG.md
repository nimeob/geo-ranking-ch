...
- ⏸️ blockiert (2026-03-03): [#914](https://github.com/nimeob/geo-ranking-ch/issues/914) — CI: stündlicher Dev-Deploy fehlgeschlagen — Root-Cause im Workflow-Smoke behoben (`set -e` + `curl`-Transportfehler beendete Retry-Loop vorzeitig); Fix in PR #917 auf `main` (Merge-Commit `1634c7c`). Verifizierungs-Run `22620539354` zeigt nun 12 saubere Retry-Versuche, scheitert aber weiterhin wegen externer Unerreichbarkeit von `https://api.dev.georanking.ch/health` (curl exit 7). Blocker liegt außerhalb dieses Issues und ist in [#912](https://github.com/nimeob/geo-ranking-ch/issues/912) / [#907](https://github.com/nimeob/geo-ranking-ch/issues/907) nachverfolgt.
