...
- Zusätzlich sind direkte Browser-Aufrufe auf API-Auth-Routen (`/auth/login`, `/auth/callback`, `/auth/logout`) auf dem API-Host fail-closed; erlaubt ist nur der UI-Proxy-Hop mit `X-Geo-Auth-Proxy: 1`.
- Hardening gegen Frontdoor-Misrouting: trifft `GET /auth/login` ohne Proxy-Marker mit `X-Forwarded-Host` = UI-Host ein, liefert der API-Service `302` auf `/login?next=...&reason=...` (UI-Reentry statt 403-JSON im Browser).
...
Für Session-Recovery nutzen `/gui` und `/gui/history` (inkl. Legacy-Redirect von `/history`) dieselben UX-Messages und denselben Redirect-Contract. Die kanonischen Recovery-Reasons sind `no_session`, `refresh_error` und `session_expired` (plus `invalid_state`/`consent_denied` für Callback-Sonderfälle):

- Session fehlt/abgelaufen (`401`, `no_session_cookie`, `session_not_found`):
  - Meldung: **„Session ungültig oder abgelaufen — bitte erneut einloggen.“**
  - Redirect: `/login?next=<current-path>&reason=no_session`
...
- Refresh fehlgeschlagen (`refresh_*`, `no_refresh_token`):
  ...
  - Redirect: `/login?next=<current-path>&reason=refresh_error`
