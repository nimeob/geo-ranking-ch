...
2. **Kartenpanel**
   ...
   - Mobile-/Tablet-Lesbarkeit ist gehärtet: Legend-Metadaten stacken bereits bei `<=820px` einspaltig (kein Text-Quetschen), auf `<=520px` werden Marker/Crosshair zusätzlich vergrößert, die Karten-Mindesthöhe reduziert und der Action-Row (`Aktuelle Position`) full-width gestapelt, damit Marker/Legenden auf kleinen Screens nicht überlappen.
3. **Result-Panel**
