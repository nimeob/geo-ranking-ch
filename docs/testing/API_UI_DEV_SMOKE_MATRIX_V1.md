...
|---|---|---|---|---|---|---|---|---|
| 1 | UI Login erfolgreich (+ Re-Login + Browser-Host-Guard) | **Ziel:** Session-basierter Login-End-to-End inkl. Re-Login bleibt stabil und leakt keinen API-Host in browser-sichtbaren Auth-Redirects.<br>**Eingangsdaten:** BFF-OIDC aktiv, gültiger OIDC Callback (`code`,`state`), Logout + erneuter Login (`next=/gui/history`, legacy `next=/history` wird kanonisiert). | **PASS:** `/auth/login` startet Redirect-Flow, Session-Cookie wird gesetzt, geschützter Flow läuft durch, Logout invalidiert Session, Re-Login funktioniert, und Login/Callback/Logout-/Error-Flow enthalten keinen API-Host im browser-sichtbaren Pfad.<br>**FAIL:** kein Redirect/Cookie, Flow bricht vor Session-Aufbau/Re-Login ab oder API-Host taucht im Browser-Auth-Flow auf. | E2E Smoke | UI/Auth | PR Gate (`pull_request`) | M (60-180s) | `tests/test_auth_regression_smoke_issue_1019.py::{test_login_search_ranking_logout_regression_smoke,test_callback_failure_modes_render_ui_relogin_without_api_host_leak,test_no_api_host_in_browser_auth_flow_guard}` |
| 2 | Geschützte Route ohne Session | **Ziel:** Default-deny auf UI-Routen bleibt stabil.<br>**Eingangsdaten:** Request auf `/gui`, `/gui/history` oder Legacy `/history` ohne gültige Session. | **PASS:** deterministischer Redirect auf `/auth/login?next=...` mit `Cache-Control: no-store`.<br>**FAIL:** 200/5xx/unklarer Redirect ohne Login-Guard. | Integration Smoke | API/Auth | PR Gate (`pull_request`) | S (<30s) | `tests/test_web_service_bff_gui_guard.py::{test_gui_redirects_to_login_when_no_session,test_legacy_history_redirect_preserves_next_query_with_canonical_successor,test_gui_history_redirect_preserves_next_query}` |
| 3 | History Happy Path im UI | **Ziel:** UI-History bleibt funktionsfähig im Data-Source-Modell.<br>**Eingangsdaten:** mindestens ein Analyze-Resultat in History. | **PASS:** `/gui/history` rendert stabil und nutzt `GET /analyze/history` gegen API-Base (Legacy `/history` redirectet auf denselben Pfad).<br>**FAIL:** History-Render bricht oder Endpoint-Bindung fehlt/falsch. | Integration | UI | PR Gate (`pull_request`) + Nightly | M (90-240s) | `tests/test_history_navigation_integration.py::test_ui_history_page_renders_and_targets_api_base` |
...
| 5 | API-Deprecation-Signal für Legacy-Auth/History/Trace | **Ziel:** Legacy-Pfade liefern klare Deprecation-/Sunset-Signale statt stiller Brüche.<br>**Eingangsdaten:** Requests auf Legacy-Routen (`/login`, `/history`, Legacy-Trace-Alias). | **PASS:** Legacy-Auth-/History-Pfade senden konsistent `Deprecation` + `Sunset` + `Warning` bei stabilen Übergangs-Statuscodes (Auth `403`, `/history` `410`, `/analyze/history` bleibt data-source-konform inkl. `400` bei invalidem Input). Legacy-Trace liefert stabil `410 gone` inkl. `Deprecation`/`Sunset`/`Warning`, `Link` mit `rel="deprecation"` + `rel="successor-version"` und Body-Hinweis `next=/debug/trace?request_id=<id>`.<br>**FAIL:** fehlende Header/unklarer Migrationshinweis/Status-Drift. | API Contract + Integration | API | PR Gate (`pull_request`) | S-M (30-120s) | Bestehend: `tests/test_auth_regression_smoke_issue_1019.py::test_login_search_ranking_logout_regression_smoke` (Legacy-Auth inkl. Alias-Checks), `tests/test_history_api_deprecation.py::{test_analyze_history_emits_deprecation_headers,test_analyze_history_validation_error_keeps_bad_request_status_with_deprecation_headers,test_history_route_returns_gone_with_deprecation_headers}` (Legacy-History + removed endpoint).<br>Ergänzt in #1211: `tests/test_trace_legacy_deprecation.py::test_trace_legacy_alias_returns_gone_with_deprecation_headers` (Legacy-Trace-Alias `/trace` -> `/debug/trace`). |
