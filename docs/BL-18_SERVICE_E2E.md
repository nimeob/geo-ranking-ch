...
Wichtige Optionen:
- `DEV_BASE_URL`: muss mit `http://` oder `https://` starten (Groß-/Kleinschreibung im Schema wird akzeptiert, z. B. `HTTP://`); Suffixe `/health` oder `/analyze` (auch verkettet) werden automatisch auf die Service-Basis normalisiert.
