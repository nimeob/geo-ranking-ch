...
def test_route_set_runner_default_fallback_uses_reports_evidence_output_dir(
    tmp_path: Path,
) -> None:
    blocker_dir = tmp_path / "blocked"
    capture_args_file = tmp_path / "fallback-default-args.txt"
    fake_bundle = tmp_path / "fake-login-start-bundle-default.sh"
    fake_bundle.write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        "printf '%s\n' \"$@\" > \"${CAPTURE_ARGS_FILE}\"\n",
    ...
    blocked_file = blocker_dir / "dev-ui-auth-analyze-smoke-blocked-manual-default-output-dir.json"
    assert blocked_file.exists()
