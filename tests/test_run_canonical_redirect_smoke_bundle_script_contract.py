...
def test_canonical_redirect_bundle_resolves_relative_paths_against_repo_root(
    tmp_path: Path,
) -> None:
    caller_cwd = tmp_path / "caller"
    caller_cwd.mkdir(parents=True, exist_ok=True)

    relative_root = Path(".tmp") / f"canonical-relative-{tmp_path.name}"
    output_dir_rel = str(relative_root / "evidence")
    summary_rel = str(relative_root / "summary" / "canonical-summary.json")
    ...
    try:
        ...
        route_artifact = (
            expected_output_dir / "stub-relative-canonical-host-redirect-smoke.json"
        )
