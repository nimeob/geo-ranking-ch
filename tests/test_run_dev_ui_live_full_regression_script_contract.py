...
def test_cli_base_url_with_gui_path_uses_single_gui_route_and_root_login_base_for_fallback(tmp_path: Path) -> None:
    repo_root = tmp_path / "mini-repo"
    script_path = repo_root / "scripts" / "run_dev_ui_live_full_regression.mjs"
    script_path.parent.mkdir(parents=True, exist_ok=True)
    script_path.write_text(SCRIPT.read_text(encoding="utf-8"), encoding="utf-8")
...
""",
    ...
    evidence_path = repo_root / "artifacts" / "dev-ui-full" / "latest" / "dev-ui-full-regression-contract-cli-base-url-gui-path-fallback.json"
    ...
    payload = json.loads(evidence_path.read_text(encoding="utf-8"))
    assert payload["ok"] is True
    assert payload["baseUrl"] == "https://www.dev.georanking.ch/gui"
    assert payload["requestedBaseUrl"] == "https://www.dev.georanking.ch/gui"
    assert payload["guiUrl"] == "https://www.dev.georanking.ch/gui"
    assert payload["degradedMode"]["active"] is True
    assert payload["degradedMode"]["envFlag"] == "cli:--fallback-login-start"
    assert "--base-url https://www.dev.georanking.ch --env-name dev" in payload["degradedMode"]["fallbackCommand"]
    assert payload["error"] in ("", None)
    assert "PASSED (degraded mode)" in result.stdout
