...
class TestWebServiceGuiMvp(unittest.TestCase):
    ...
    def test_gui_shell_exposes_state_machine_markers(self):
        ...
        self.assertIn('ui.view.error_view.server_5xx', body)
        self.assertIn('function emitServerErrorViewShown(viewState = {})', body)
        ...
        self.assertIn('Support mit Referenz-ID kontaktieren.', body)
        self.assertIn('segments.push(`Referenz-ID: ${requestId}`);', body)
        ...
        self.assertIn('function triggerResultsListLoginRecovery(authReason = "")', body)
        ...
        self.assertIn('const HISTORY_AUTH_MIGRATION_NOTICE = "Historische Abfragen sind in dieser Umgebung aktuell noch nicht über die GUI-Session verfügbar.";', body)
        self.assertIn('let historyPanelFetchSupported = true;', body)
        self.assertIn('let historyPanelFetchCompatibilityNotice = "";', body)
        self.assertIn('function renderHistoryCompatibilityNotice(message)', body)
        self.assertIn('if (!historyPanelFetchSupported) {', body)
        ...
        self.assertIn('if (isDocumentHidden()) {', body)
        self.assertIn('const AUTH_UNAUTHENTICATED_BACKGROUND_POLL_COOLDOWN_MS = 300000;', body)
        self.assertIn('nextUnauthenticatedPollAtMs: 0,', body)
        self.assertIn('let authSessionRefreshInFlight = null;', body)
        self.assertIn('if (authSessionRefreshInFlight) {', body)
        self.assertIn('authSessionRefreshInFlight = refreshTask;', body)
        self.assertIn('authState.authenticated === false', body)
        self.assertIn('authState.nextUnauthenticatedPollAtMs = Date.now() + AUTH_UNAUTHENTICATED_BACKGROUND_POLL_COOLDOWN_MS;', body)
        self.assertIn('function isDocumentHidden()', body)
        self.assertIn('document.visibilityState === "hidden"', body)
        self.assertIn('if (!force && authState.authenticated === false && isDocumentHidden()) {', body)
        self.assertIn('function scheduleAuthVisibilityRefresh()', body)
        self.assertIn('document.addEventListener("visibilitychange", () => {', body)
        self.assertIn('const forceRefresh = authState.authenticated !== true;', body)
        self.assertIn('scheduleAuthVisibilityRefresh();', body)
        self.assertIn('const forceRefresh = authState.authenticated === true;', body)
        self.assertIn('window.location.hash || ""', body)
        self.assertIn('session_expires_at', body)
        self.assertIn('session_expires_in_seconds', body)
        self.assertIn('function parseSessionExpiryInSeconds(rawValue)', body)
        self.assertIn('const remainingMs = nextExpiryInSeconds > 0', body)
        self.assertIn('function resolveAuthFailure(statusCode, errorCode, fallbackMessage)', body)
        self.assertIn('function buildTraceFeatureFlagHint(errorCode, statusCode, responsePayload)', body)
        self.assertIn('normalizedCode === "debug_trace_disabled"', body)
        self.assertIn('TRACE_DEBUG_ENABLED=1 setzen', body)
        ...
        self.assertIn('const traceFeatureHint = buildTraceFeatureFlagHint(authFailure.errorCode, response.status, parsed);', body)
        self.assertIn('errorMessage: withTechnicalRequestIdHint(traceFeatureHint || authFailure.errorMessage, responseRequestId),', body)
        ...
        self.assertIn('no_session_cookie: "no_session"', body)
        self.assertIn('refresh_grant_error: "refresh_error"', body)
        self.assertIn('function resolveDevErrorCode(errorCode, statusCode)', body)
        self.assertIn('error_code: resolveDevErrorCode(errorCode, statusCode),', body)
        self.assertIn('const authenticated = await ensureAuthenticatedForAction({', body)
        self.assertIn('rows.join("\\n")', body)
        self.assertIn('const deprecationScope = data && data.deprecation && data.deprecation.scope', body)
        self.assertIn('const missingBearerToken = responseMessage.toLowerCase().includes("missing or invalid bearer token");', body)
        self.assertIn('historyPanelFetchSupported = false;', body)
        self.assertIn('historyPanelFetchCompatibilityNotice = HISTORY_AUTH_MIGRATION_NOTICE;', body)
        self.assertIn('historyPanelFetchCompatibilityNotice = "";', body)
        self.assertIn('renderHistoryCompatibilityNotice(historyPanelFetchCompatibilityNotice);', body)
        ...
        self.assertIn('requestTimeoutMs: 12000', body)
        self.assertIn('maxRetryBudget: 1', body)
        self.assertIn('retryDelayMs: 250', body)
        ...
        self.assertIn('function resolveDevRequestTimeoutMs(options = {})', body)
        self.assertIn('function resolveDevRetryBudget(options = {})', body)
        self.assertIn('function summarizeDevRequestFailure(result = {})', body)
        self.assertIn('function formatDevRequestFailureMessage(contextLabel, summary, { includeTimeoutSeconds = false } = {})', body)
        ...
        self.assertIn('attempts=${summary.attemptCount}', body)
        self.assertIn('final_reason=${summary.finalReason}', body)
        ...
        self.assertIn('function resolveRequestCorrelationId(headers)', body)
        self.assertIn('preparedHeaders.set("X-Request-Id", requestId);', body)
        self.assertIn('preparedHeaders.set("X-Correlation-Id", requestId);', body)
        self.assertIn('headers["X-Request-Id"] = requestId;', body)
        self.assertIn('headers["X-Correlation-Id"] = requestId;', body)
        ...
        self.assertIn('response: projectedResponse,', body)
    ...
    def test_gui_map_marker_legibility_styles_present(self):
        ...
        self.assertIn("grid-template-columns: repeat(auto-fit, minmax(14rem, 1fr))", body)
        self.assertIn(".map-legend > *", body)
        self.assertIn("min-height: 2.15rem", body)
    ...
    def test_gui_map_legend_mobile_overlap_contract_present(self):
        ...
        self.assertIn("background: rgba(255, 255, 255, 0.86)", body)
        self.assertIn(".map-legend--meta #click-hint", body)
        self.assertIn(".map-legend--actions #map-location-meta", body)
        self.assertIn(".map-legend--actions .map-locate-btn", body)
        self.assertIn("@media (max-width: 820px)", body)
        self.assertIn(".map-user-marker {", body)
        self.assertIn("width: 18px", body)
        self.assertIn("line-height: 1.4", body)
    ...
    def test_gui_mobile_touch_target_css_contract_present(self):
        ...
        self.assertIn(".results-filters-actions-primary", body)
        self.assertIn(".results-filters-actions-secondary", body)
        self.assertIn("--results-filters-action-reserve", body)
