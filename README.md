...
**Request-Korrelation:** Für `POST /analyze` wird die **erste gültige** ID aus `X-Request-Id` (primär) bzw. `X-Correlation-Id` (Fallback) in die Antwort gespiegelt (`X-Request-Id` Header + JSON-Feld `request_id`). Leere/whitespace-only IDs, IDs mit eingebettetem Whitespace sowie IDs mit Steuerzeichen werden verworfen; ohne gültige Header-ID erzeugt der Service automatisch eine Request-ID.
