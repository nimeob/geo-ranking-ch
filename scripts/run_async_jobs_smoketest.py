...
def _normalize_classification(value: str) -> str:
    classification = _strip(value).lower() or "must-pass"
    if classification not in {"must-pass", "informational"}:
        raise ValueError("classification must be must-pass or informational")
    return classification


def _normalize_smoke_test_seed(value: str) -> str:
    seed = _strip(value)
    if not seed:
        return ""
    if _has_control_chars(seed):
        raise ValueError("dev-smoke-test-seed must not contain control chars")
    return seed


def _build_seeded_request_id(*, seed: str, base_url: str, query: str, mode: str, timeout_seconds: float) -> str:
    basis = "|".join(
        [
            seed,
            base_url,
    ...
    return f"async-smoke-seed-{digest}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dev-smoke-test-seed",
        default=_strip(os.getenv("DEV_SMOKE_TEST_SEED", "")),
        help="Optional deterministic seed for stable autogenerated request IDs",
    )
    parser.add_argument(
        "--classification",
        default=_strip(os.getenv("ASYNC_SMOKE_CLASSIFICATION", os.getenv("SMOKE_CLASSIFICATION", "must-pass"))),
        help="Smoke classification (must-pass|informational)",
    )
    return parser
...
def _resolve_config(args: argparse.Namespace) -> SmokeConfig:
    ...
    smoke_test_seed = _normalize_smoke_test_seed(str(args.dev_smoke_test_seed))

    request_id = _strip(str(args.request_id))
    if not request_id:
        if smoke_test_seed:
            request_id = _build_seeded_request_id(
    ...
    classification = _normalize_classification(str(args.classification))
...
def main(argv: list[str]) -> int:
    ...
    if cfg.smoke_test_seed:
        print(f"[async-jobs-smoke] deterministic seed: {cfg.smoke_test_seed}")
    ...
    evidence = {
        ...
        "runner": "remote-async-jobs-smoke",
        ...
        "dev_smoke_test_seed": cfg.smoke_test_seed or None,
        "submit": {
            "url": analyze_url,
